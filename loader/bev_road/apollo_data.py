    ...
    def get_y_offset_and_z(self,res_d):
        ...
        for idx in res_d:
            ...
            base_points_bin = np.linspace(int(x.min()), int(x.max()),
                                          int(int(x.max()) - int(x.min()))+1)  # .astype(np.int64)
            ...
            # cv2.polylines(instance_seg, [ipm_points.T.astype(np.int64)], False, idx+1, 1)
            res_lane_points[idx] = np.array([base_points, y_points])  # 
            res_lane_points_z[idx] = np.array([base_points, z_points])
            res_lane_points_bin[idx] = np.array([base_points_bin, y_points_bin]).astype(np.int64)  # 画bin用的
